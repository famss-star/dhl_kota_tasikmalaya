...
export default function CreatePerizinanAmdal() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/perizinan/amdal", {
        method: "POST",
        headers: {
      ...
      if (data.success) {
        alert("AMDAL berhasil dibuat!");
        router.push("/admin/perizinan/amdal");
      } else {
        alert(data.error || "Gagal membuat AMDAL");
      }
    } catch (error) {
      console.error("Create AMDAL error:", error);
      alert("Terjadi kesalahan saat membuat AMDAL");
    } finally {
      setLoading(false);
    }
  };
