...
import { SPPLData } from "@/types/perizinan";
...
export default function AdminPerizinanSPPLPage() {
  const router = useRouter();
  const { data: spplData, isLoading, handleDelete } = usePerizinanData<SPPLData>("/api/perizinan/sppl");
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            Kelola data perizinan SPPL
          </p>
        </div>
      </div>

      <StatisticsCards stats={stats} title="SPPL" />
        ...
        onAdd={() => router.push("/admin/perizinan/sppl/create")}
        addButtonText="Tambah SPPL"
      />

      {filteredData.length === 0 ? (
        <EmptyState 
          icon={FileCheck}
          title="Tidak ada data SPPL"
          description="Mulai dengan menambahkan data SPPL baru."
        />
      ) : (
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {filteredData.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900 dark:text-white">
                        {(item.luas_area || 0).toLocaleString()} m²
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-500 dark:text-gray-400">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <div className="flex items-center justify-center gap-2">
                        <button
                          onClick={() => router.push(`/admin/perizinan/sppl/${item.id}`)}
                        ...
                        <button
                          onClick={() => router.push(`/admin/perizinan/sppl/edit/${item.id}`)}
